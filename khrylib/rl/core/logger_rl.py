...
class LoggerRL:
    ...
    def __init__(self, init_stats_logger=True):
        ...
        self.num_episodes = 0
        self.sample_time = 0
        self.stats_names = ['episode_len', 'reward', 'episode_reward','episode_cost','episode_od','episode_addod']
    ...
    def end_episode(self, info):
        ...
        self.stats_loggers['episode_cost'].log(info['cost'])
        self.stats_loggers['episode_od'].log(info['od'])
        self.stats_loggers['episode_addod'].log(info['add_od'])
    ...
    def merge(cls, logger_list, **kwargs):
        ...
        logger.total_reward = logger.stats_loggers['episode_od'].total()
        logger.avg_episode_len = logger.stats_loggers['episode_len'].avg()
        logger.avg_episode_reward = logger.stats_loggers['episode_od'].avg()
        logger.cost = logger.stats_loggers['episode_cost'].avg()
        logger.od = logger.stats_loggers['episode_od'].avg()
        logger.addod = logger.stats_loggers['episode_addod'].avg()
