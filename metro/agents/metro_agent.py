...
from metro.envs import MetroEnv
from metro.models.model import create_RL_model, create_GNN1_model, create_GNN2_model, create_GNN3_model, ActorCritic
from metro.models.baseline import RandomPolicy, GreedyPolicy, NullModel
from metro.utils.tools import TrajBatchDisc
...
class MetroExpansionAgent(AgentPPO):
    ...
    def setup_env(self):
        self.env = env = MetroEnv(self.cfg)
    ...
    def log_optimize_policy(self, iteration, info):
        ...
        tb_logger.add_scalar('train/train_R_eps_avg',
                             ...
                             iteration)
        tb_logger.add_scalar('train/total_cost', log.cost,
                             iteration)
        tb_logger.add_scalar('train/total_od', log.od, iteration)
        tb_logger.add_scalar('train/add_od', log.addod, iteration)

        tb_logger.add_scalar('eval/eval_R_eps_avg',
                             log_eval.avg_episode_reward + self.reward_offset,
                             iteration)
        tb_logger.add_scalar('eval/total_cost',
                             log_eval.cost, iteration)
        tb_logger.add_scalar('eval/total_od',
                             log_eval.od, iteration)
        
        tb_logger.add_scalar('eval/add_od',
                        log_eval.addod, iteration)


    def eval_agent(self, num_samples=1, mean_action=True, visualize=True):
        t_start = time.time()
        ...
        with to_cpu(*self.sample_modules):
            with torch.no_grad():
                logger = self.logger_cls(**self.logger_kwargs)
                while logger.num_steps < num_samples:
                    state = self.env.reset(eval=True)
                    if visualize:
    ...
    def infer(self,
        ...
        logger.info(f'od: {log_eval.od}')
        logger.info(f'cost: {log_eval.cost}')
